...
def ensure_database():
    """DBが無ければ初期化＋シード。マスタが空なら再シード。プロセス内1回のみ。"""
    ...
    if not db_path.exists():
        ...
        try:
            cursor.execute("SELECT COUNT(*) as cnt FROM reactions_master")
            row = cursor.fetchone()
            if row["cnt"] == 0:
                print("[DB] reactions_master が空。シードデータを投入。")
                needs_seed = True
        except Exception:
            conn_check.close()
            init_database()
    ...
    if needs_seed:
        try:
            import sys as _sys
            seed_path = str(Path(__file__).parent.parent / "scripts")
            if seed_path not in _sys.path:
                _sys.path.insert(0, seed_path)
            from seed_master_data import seed_all
            seed_all()
        except ImportError:
            print("[DB] seed_master_data.py が見つかりません。")
...
def init_database():
    ...
    # チャームプロファイルテーブル
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS charm_profiles (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            session_id TEXT,
            person_name TEXT,
            ...
            personality_shadow TEXT,
            scores_json TEXT NOT NULL,
            emotion_shelf_json TEXT NOT NULL,
            full_profile_json TEXT,
            reaction_hints_json TEXT,
            raw_observations_json TEXT,
            created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
            FOREIGN KEY (session_id) REFERENCES sessions(id)
        )
    """)

    # charm_profiles マイグレーション
    try:
        cursor.execute("ALTER TABLE charm_profiles ADD COLUMN person_name TEXT")
    except sqlite3.OperationalError:
        pass
    try:
        cursor.execute("ALTER TABLE charm_profiles ADD COLUMN full_profile_json TEXT")
    except sqlite3.OperationalError:
        pass
    ...
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS session_qa (
            ...
            step TEXT NOT NULL,
            question_key TEXT,
            question_type TEXT NOT NULL DEFAULT 'freetext',
    ...
    # チャームプロファイル用インデックス
    cursor.execute("CREATE INDEX IF NOT EXISTS idx_charm_profiles_session ON charm_profiles(session_id)")
    cursor.execute("CREATE INDEX IF NOT EXISTS idx_charm_profiles_person ON charm_profiles(person_name)")

    # Q&A記録用インデックス
    cursor.execute("CREATE INDEX IF NOT EXISTS idx_session_qa_session ON session_qa(session_id)")
    cursor.execute("CREATE INDEX IF NOT EXISTS idx_session_qa_step ON session_qa(step)")
...
def select_reactions_for_persona(
    ...
    # リアクションを選択
    # source='custom' はキャラ固有のため除外（--include-custom で明示的に含める）
    cursor.execute("""
        SELECT
            rm.*,
...
def select_fallback_reactions(exclude_ids: list = None, limit: int = 24) -> List[Dict]:
    """ペルソナフィルタなしでフォールバックリアクションを取得

    DBが返す件数が不足した際の補完用。全リアクションから
    essential優先・priority順で取得する。
    """
    conn = get_connection()
    cursor = conn.cursor()
    ...
    if exclude_ids:
        ...
        exclude_clause = f"AND rm.id NOT IN ({placeholders})"
    ...
    cursor.execute(f"""
        ...
        JOIN pose_master pm ON rm.pose_id = pm.id
        JOIN text_master tm ON rm.text_id = tm.id
        WHERE rm.source = 'builtin' {exclude_clause}
        ORDER BY rm.is_essential DESC, rm.priority DESC
        LIMIT ?
    """, params)
    ...
    for row in rows:
        data = dict(row)
        for key in ['persona_age', 'persona_target', 'persona_theme',
                    'intensity_range', 'incompatible_reactions', 'text_variants']:
            if data.get(key):
                try:
                    data[key] = json.loads(data[key])
                except (json.JSONDecodeError, TypeError):
...
def save_charm_profile(
    ...
    cursor.execute("""
        INSERT INTO charm_profiles (
            session_id, person_name, input_mode, core_charm,
            personality_primary, personality_shadow,
            scores_json, emotion_shelf_json,
            full_profile_json,
            reaction_hints_json, raw_observations_json
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    """, (
        session_id,
        person_name,
        charm_data.get("mode", "light"),
        charm_data.get("core_charm"),
        charm_data.get("personality_type", {}).get("primary"),
        charm_data.get("personality_type", {}).get("shadow"),
        json.dumps(charm_data.get("scores", {}), ensure_ascii=False),
        json.dumps(charm_data.get("emotion_shelf", {}), ensure_ascii=False),
        json.dumps(charm_data, ensure_ascii=False),
        json.dumps(charm_data.get("reaction_style", {}), ensure_ascii=False),
        json.dumps(charm_data.get("raw_observations", []), ensure_ascii=False),
    ))
...
def get_charm_profile(session_id: str) -> Optional[Dict]:
    """セッションに紐づくチャームプロファイルを取得"""
    conn = get_connection()
    cursor = conn.cursor()

    cursor.execute(
        "SELECT * FROM charm_profiles WHERE session_id = ? ORDER BY created_at DESC LIMIT 1",
        (session_id,)
    )
    row = cursor.fetchone()
    conn.close()
    ...
    data = dict(row)
    for key in ['scores_json', 'emotion_shelf_json', 'reaction_hints_json', 'raw_observations_json', 'full_profile_json']:
        if data.get(key):
            try:
                data[key] = json.loads(data[key])
            except (json.JSONDecodeError, TypeError):
...
def list_charm_profiles(limit: int = 20) -> List[Dict]:
    """保存済みチャームプロファイル一覧を取得"""
    conn = get_connection()
    cursor = conn.cursor()

    cursor.execute("""
        SELECT id, person_name, core_charm, personality_primary, personality_shadow,
               input_mode, session_id, created_at
        FROM charm_profiles
...
def get_charm_profile_by_name(person_name: str) -> Optional[Dict]:
    """人物名でチャームプロファイルを取得（最新のもの）"""
    conn = get_connection()
    cursor = conn.cursor()

    cursor.execute(
        "SELECT * FROM charm_profiles WHERE person_name = ? ORDER BY created_at DESC LIMIT 1",
        (person_name,)
    )
    row = cursor.fetchone()
    conn.close()
    ...
    data = dict(row)
    # full_profile_json があればそれを返す（完全な charm.json を復元可能）
    if data.get("full_profile_json"):
        try:
            return json.loads(data["full_profile_json"]) if isinstance(data["full_profile_json"], str) else data["full_profile_json"]
        except (json.JSONDecodeError, TypeError):
            pass

    # フォールバック: 個別カラムから再構成
    for key in ['scores_json', 'emotion_shelf_json', 'reaction_hints_json', 'raw_observations_json']:
        if data.get(key):
            try:
                data[key] = json.loads(data[key])
            except (json.JSONDecodeError, TypeError):
...
def record_qa(
    ...
    cursor.execute("""
        INSERT INTO session_qa (
            session_id, step, question_key, question_type,
            question_text, answer_text, answer_json
        ) VALUES (?, ?, ?, ?, ?, ?, ?)
    """, (
        session_id, step, question_key, question_type,
        question_text, answer_text, answer_json,
...
def record_qa_batch(
    ...
    for entry in entries:
        ...
        """, (
            ...
            entry.get("question_type", "freetext"),
            entry["question_text"],
            entry.get("answer_text"),
            entry.get("answer_json"),
        ))
...
def get_session_qa(
    ...
    """セッションのQ&A履歴を取得（step指定でフィルタ可）"""
    conn = get_connection()
    cursor = conn.cursor()

    if step:
        cursor.execute(
            "SELECT * FROM session_qa WHERE session_id = ? AND step = ? ORDER BY created_at",
            (session_id, step),
        )
    else:
        cursor.execute(
            "SELECT * FROM session_qa WHERE session_id = ? ORDER BY created_at",
            (session_id,),
        )
    ...
    for row in rows:
        data = dict(row)
        if data.get("answer_json"):
            try:
                data["answer_json"] = json.loads(data["answer_json"])
            except (json.JSONDecodeError, TypeError):
                pass
        results.append(data)
...
def get_session_qa_summary(session_id: str) -> Dict[str, int]:
    """ステップごとのQ&A件数サマリーを返す"""
    conn = get_connection()
    cursor = conn.cursor()

    cursor.execute(
        "SELECT step, COUNT(*) as cnt FROM session_qa WHERE session_id = ? GROUP BY step",
        (session_id,),
    )
