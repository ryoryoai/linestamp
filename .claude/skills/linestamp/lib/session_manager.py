...
from database import (
    ...
    save_template,
    record_qa as db_record_qa,
    record_qa_batch as db_record_qa_batch,
    get_session_qa as db_get_session_qa,
)
...
class Session:
    ...
    def record_qa(self, step: str, question_text: str, answer_text: str, **kwargs) -> int:
        """Q&Aを1件記録"""
        return db_record_qa(self.session_id, step, question_text, answer_text, **kwargs)

    def record_qa_batch(self, step: str, entries: List[Dict]) -> list:
        """複数のQ&Aを一括記録"""
        return db_record_qa_batch(self.session_id, step, entries)

    def get_qa_history(self, step: str = None) -> list:
        """Q&A履歴を取得"""
        return db_get_session_qa(self.session_id, step=step)
